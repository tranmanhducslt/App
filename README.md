...
The benchmark program tests different values of lookahead and returns the result in an `.html` file. 
...
```
* Information is stored in `.csv` files, which is connected to the `.html` file that can be opened in a supporting web browser (e.g., Firefox or Chrome).
